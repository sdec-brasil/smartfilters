...
function validateSettlements() {
  function defineReceiver(invoice) {
    const tomador = [];
    const provedor = [];
    while (invoice.length) {
      const element = invoice.shift();
      const { serviceCode } = invoice.provision;
      const prefixCode = `${serviceCode.split('.')[0]}.${serviceCode.split('.')[1]}`;
      if (lc116[prefixCode].incidenciaTomador) {
